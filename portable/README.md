...
where `<InitAmount>` and `<MaxAmount>` indicate initial and maximum amount of heap space (in MB) respectively and are thus to be replaced by concrete non-zero integers.
 ...
 Option | Meaning | Default value
 ---- | --- | ---
 `MATCH_ONLY` | Match only specific types of elements and ignore the rest. If more than one type are given, they must be separated by a space letter (` `). Note that this only applies to children (first descendants) of buildings (e.g. `APPEARANCE`). Also note that the mapping process still maps the entire input CityGML datasets regardless of this option. | 
 `APPEARANCE_LOCATION` | Specify the default location of textures in case they are to be uploaded to the WFS. The location must be accessible by the WFS, such as as an HTTP-URL (e.g. `http://localhost/`) or an absolute path (e.g. `C:\`). Note that this option must end with either `/` or `\` depending on the given platform. | 
